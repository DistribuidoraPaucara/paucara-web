...
export function ApprovalPaymentForm({
    ...
    return (
        <div className="space-y-6 py-4">
            ...
            {/* ✅ NUEVO (2026-06-09): Resumen de Crédito del Cliente */}
            {proforma.politica_pago === 'CREDITO' && (
                <Card className={resumenCredito?.advertencia ? 'border-amber-200 dark:border-amber-800' : ''}>
                    <CardHeader className={resumenCredito?.advertencia ? 'bg-amber-50 dark:bg-amber-950/30' : ''}>
                    ...
                    <CardContent className="space-y-4">
                        {loadingResumenCredito ? (
                            <div className="flex items-center justify-center py-6">
                                ...
                                <span className="text-sm text-muted-foreground">Cargando información de crédito...</span>
                            </div>
                        ) : resumenCredito ? (
                            <>
                                {/* Tabla con información de crédito */}
                                <div className="space-y-3 text-sm">
                                    ...
                                    <div className="flex justify-between p-2 bg-slate-50 dark:bg-slate-800/50 rounded">
                                        ...
                                        <span className="font-semibold">
                                            Bs. {resumenCredito.limite_credito?.toLocaleString('es-ES', { minimumFractionDigits: 2 })}
                                        </span>
                                    </div>

                                    {/* Monto Utilizado */}
                                    <div className="flex justify-between p-2 bg-red-50 dark:bg-red-900/20 rounded">
                                        <span className="text-muted-foreground">Utilizado (CxC):</span>
                                        <span className="font-semibold text-red-600 dark:text-red-400">
                                            Bs. {resumenCredito.monto_utilizado?.toLocaleString('es-ES', { minimumFractionDigits: 2 })}
                                        </span>
                                    </div>

                                    {/* Intenciones (Proformas Pendientes) */}
                                    <div className="flex justify-between p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded">
                                        <span className="text-muted-foreground">Intenciones (Pendientes):</span>
                                        <span className="font-semibold text-yellow-700 dark:text-yellow-400">
                                            Bs. {resumenCredito.total_intenciones?.toLocaleString('es-ES', { minimumFractionDigits: 2 })}
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex justify-between p-2 bg-green-50 dark:bg-green-900/20 rounded font-semibold">
                                        <span className="text-green-900 dark:text-green-200">Disponible:</span>
                                        <span className="text-green-700 dark:text-green-400">
                                            Bs. {resumenCredito.disponible?.toLocaleString('es-ES', { minimumFractionDigits: 2 })}
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex justify-between p-2 bg-blue-50 dark:bg-blue-900/20 rounded">
                                        ...
                                        <span className="font-semibold text-blue-700 dark:text-blue-400">
                                            Bs. {resumenCredito.monto_proforma_actual?.toLocaleString('es-ES', { minimumFractionDigits: 2 })}
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex justify-between p-2 bg-purple-50 dark:bg-purple-900/20 rounded">
                                        <span className="text-muted-foreground">Quedaría Disponible:</span>
                                        <span className={`font-semibold ${
                                            resumenCredito.puede_convertir
                                                ? 'text-green-600 dark:text-green-400'
                                                : 'text-red-600 dark:text-red-400'
                                        }`}>
                                            Bs. {resumenCredito.quedaria_disponible?.toLocaleString('es-ES', { minimumFractionDigits: 2 })}
                                        </span>
                                    </div>
                                </div>
                                ...
                                {resumenCredito.proformas_pendientes?.length > 0 && (
                                    <div className="mt-4">
                                        <p className="text-xs font-semibold text-muted-foreground mb-2">📋 Proformas Pendientes con Crédito:</p>
                                        <div className="space-y-1 max-h-[150px] overflow-y-auto">
                                            {resumenCredito.proformas_pendientes.map((pf: any, idx: number) => (
                                                <div key={idx} className="text-xs flex justify-between p-1 bg-gray-50 dark:bg-gray-800 rounded">
                                                    <span>{pf.numero}</span>
                                                    <span className="font-mono">Bs. {pf.total.toLocaleString('es-ES', { minimumFractionDigits: 2 })}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                {!resumenCredito.puede_convertir && (
                                    <Alert className="border-red-300 bg-red-50 dark:border-red-800 dark:bg-red-950">
                                        ...
                                        <AlertDescription className="text-red-800 dark:text-red-200 text-xs">
                                            🚫 Límite de crédito insuficiente. No se puede convertir esta venta.
                                        </AlertDescription>
                                    </Alert>
                                )}
                            </>
                        ) : (
                            <p className="text-sm text-muted-foreground">No se pudo cargar la información de crédito</p>
                        )}
                    </CardContent>
                </Card>
            )}
