...
interface DevolucionData {
    ...
    detalles: Array<{
        prestamo_evento_detalle_id: number;
...
export default function DevolucionesEventos({ prestamoId }: Props) {
        ...
        try {
            ...
            setDevolucionData(prev => ({
                ...
                detalles: data.detalles.map((d: any) => ({
                    prestamo_evento_detalle_id: d.id,
                    cantidad_devuelta: 0,
    ...
    useEffect(() => {
        ...
        const montoTotalDanios = devolucionData.detalles.reduce((sum, det) => {
            const detallePrestamo = prestamo.detalles?.find((d: any) => d.id === det.prestamo_evento_detalle_id);
        ...
        try {
            const payload = {
                ...
                detalles: devolucionData.detalles.map((d) => ({
                    prestamo_evento_detalle_id: d.prestamo_evento_detalle_id,
            ...
            await prestamoEventoService.devolver(prestamo.id, payload as any);
    ...
    return (
        <AppLayout>
            <Head title="Registrar Devolución - Evento" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <Card className="p-3">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-xs">Encargado Evento</CardTitle>
                        </CardHeader>
                    ...
                    <Card className="p-3">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-xs">Chofer</CardTitle>
                        </CardHeader>
                        <CardContent className="pt-0">
                            <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">
                                {prestamo.chofer?.name || '-'}
                            </p>
                        </CardContent>
                    </Card>
                    <Card className="p-3">
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {detalles.map((detalle: any) => {
                                            const detalleAct = devolucionData.detalles.find(d => d.prestamo_evento_detalle_id === detalle.id);
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) => {
                                                                ...
                                                                let detallesActualizados = devolucionData.detalles.map(d =>
                                                                    d.prestamo_evento_detalle_id === detalle.id
                                                                ...
                                                                if (detalle.prestable?.tipo === 'CANASTILLA') {
                                                                    ...
                                                                    if (detalleEmbase) {
                                                                        ...
                                                                        detallesActualizados = detallesActualizados.map(d =>
                                                                            d.prestamo_evento_detalle_id === detalleEmbase.id
