...
import type { PrestamoProveedor } from '@/domain/entities/prestamos';
...
export default function RegistrarDevolucionProveedor({ prestamoId }: Props) {
    ...
    const [prestamo, setPrestamo] = useState<PrestamoProveedor | null>(null);
    ...
    const cargarPrestamo = async () => {
        setLoading(true);
        try {
            const data = await prestamoProveedorService.obtener(prestamoId);
            setPrestamo(data);
            // Inicializar detalles de devolución
            setDevolucionData(prev => ({
                ...prev,
                detalles: data.detalles.map((d: any) => ({
                    prestamo_proveedor_detalle_id: d.id,
                    cantidad_devuelta: 0,
    ...
    useEffect(() => {
        ...
        const montoTotalDanios = devolucionData.detalles.reduce((sum, det) => {
            const detallePrestamo = prestamo.detalles?.find((d: any) => d.id === det.prestamo_proveedor_detalle_id);
        ...
        try {
            const payload = {
                ...
                detalles: devolucionData.detalles.map((d) => ({
                    prestamo_proveedor_detalle_id: d.prestamo_proveedor_detalle_id,
            ...
            await prestamoProveedorService.devolver(prestamo.id, payload as any);
    ...
    return (
        <AppLayout>
            <Head title="Registrar Devolución - Proveedor" />
            ...
            <div className="space-y-6 p-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-600 dark:text-gray-400 mt-1">
                            {prestamo.proveedor?.nombre || prestamo.proveedor?.razon_social}
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <Card className="p-3">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-xs">Proveedor</CardTitle>
                        </CardHeader>
                        <CardContent className="pt-0">
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) => {
                                                                ...
                                                                let detallesActualizados = devolucionData.detalles.map(d =>
                                                                    d.prestamo_proveedor_detalle_id === detalle.id
                                                                ...
                                                                if (detalle.prestable?.tipo === 'CANASTILLA') {
                                                                    ...
                                                                    if (detalleEmbase) {
                                                                        ...
                                                                        detallesActualizados = detallesActualizados.map(d =>
                                                                            d.prestamo_proveedor_detalle_id === detalleEmbase.id
